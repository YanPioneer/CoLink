...
class CMeKGRetriever:
    ...
    def precompute_and_save_embeddings(self):
        ...
        print("开始预计算所有节点的嵌入...")
        # 获取所有拥有 'name' 属性的节点名称
        all_nodes = self.run_query("MATCH (n) WHERE n.name IS NOT NULL RETURN n.name AS name")
        
        if not all_nodes:
            print("错误：数据库中没有找到任何带有 'name' 属性的节点。请确认数据已正确导入。")
            return

        unique_node_names = sorted(list(set(n['name'] for n in all_nodes if n['name'])))
    ...
    def query_by_entities(self, entities):
        """根据实体名称列表，查询一度关系。"""
        if not entities: 
            return []
        # 基于 'name' 属性和动态关系类型
        query = """
        MATCH (h)-[r]->(t) WHERE h.name IN $entities RETURN h.name AS head, type(r) AS relation, t.name AS tail
        UNION
        MATCH (h)-[r]->(t) WHERE t.name IN $entities RETURN h.name AS head, type(r) AS relation, t.name AS tail
        """
        return self.run_query(query, {"entities": entities})


    def query_multihop_by_entities(self, entities, max_hops=2):
        """根据实体名称列表进行多跳查询。"""
        if not entities: 
            return []
        ...
        RETURN DISTINCT startNode(rel).name AS head, type(rel) AS relation, endNode(rel).name AS tail
        """
        
        return self.run_query(query, {"entities": entities})


    def retrieve_kg_triples(self, text, hops=1, use_semantic_search=True):
        """综合检索入口。"""
        entities = [text]  #❗修改实体抽取方法
        # entities = extract_entities_medical(text=text, client=client_A)
        # if not entities:
        #     return []

        triples = []
        print(f"🔍 开始精确匹配 '{text}' (在 {hops} 跳内)...")
        if hops > 1:
            triples = self.query_multihop_by_entities(entities, max_hops=hops)
        else:
            triples = self.query_by_entities(entities)

        if not triples and use_semantic_search:
            print(f"  -> 无精确匹配结果，尝试语义检索...")
            similar_nodes_with_scores = self.semantic_search(text)
            
            similar_nodes = [r['term'] for r in similar_nodes_with_scores]
            similar_scores = [r['score'] for r in similar_nodes_with_scores]
            ...
            if hops > 1:
                triples = self.query_multihop_by_entities(similar_nodes, max_hops=hops)
            else:
                triples = self.query_by_entities(similar_nodes)
        
        unique_triples = [dict(t) for t in {tuple(d.items()) for d in triples}]
        return json.dumps(unique_triples, ensure_ascii=False, indent=2)
