...
def main():
    ...
    if not os.path.exists(NODE_EMBS_PATH):
        ...
        retriever_gen = WIKGRetriever2(load_embeddings_on_init=False)
    ...
    if os.path.exists(NODE_EMBS_PATH):
        ...
        data_path = os.path.join(gfrand_path, 'Data', 'medical_data_v3_eval.json')
        save_path = os.path.join(gfrand_path, 'Data', 'medical_data_v3_eval_kgpath_gpt4o_mini_alpha_06_samplekg_final.json')
        ...
        for i in range(len(dialog_data)):
            ...
            dialog_history.append(dialog_data[i]['current_q'])
            current_q = dialog_data[i]['current_q']
            conversation_list = dialog_history
