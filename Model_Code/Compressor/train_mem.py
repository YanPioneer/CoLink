...
class DialogueDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'response' in item:
            ...
            current_text = history[-2]
            summary_text = self._generate_summary(history)
            response_text = current_text  # 使用当前对话作为回复
        ...
        summary_encoding = self.tokenizer(
         ...
         # 1. 在文本层面拼接，使用Qwen的标准对话格式（包含system）
        # system部分将被slots的embeddings替换
        # qy_text = f"<|im_start|>system\n历史对话摘要<|im_end|>\n<|im_start|>user\n{current_text}<|im_end|>\n<|im_start|>assistant\n{response_text}<|im_end|>"
        qy_text = f"{current_text}{response_text}"
        
        # 2. 一次性tokenize（这样可以控制总长度）
        ...
        # 暂时不使用特殊token，保持简单
        # 后续版本可以考虑加入<|im_start|>等特殊token
        
        # 1. 分别tokenize q和y（不加特殊token）
        ...
        q_length = len(q_ids)  # 记录q的长度，用于创建labels
        
        # 3. Padding到固定长度
        qy_len = len(qy_ids_concat)
        if qy_len < self.max_len_qy:
            # 需要padding
        ...
        # qy_ids = qy_encoding['input_ids'].to(self.device)
        ...
        with torch.no_grad():
            ...
            q_embeddings = self.model.model.embed_tokens(current_input_ids)
            qy_embeddings = self.model.model.embed_tokens(qy_ids.unsqueeze(0))
        ...
        return {
            ...
            'q_length': q_length,  # q部分的长度，用于创建labels
...
def train_compressor(compressor, train_loader, optimizer, device, qwen_model, tokenizer, compressor_name, num_epochs=10, logger=None, save_dir=None, model_config=None, save_interval=2):
    """训练压缩模型"""
    compressor.train()
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # 获取文本数据（用于预测损失）
            current_text = batch.get('current_text', [])  # 从batch中获取当前问题文本
            response_text = batch.get('response_text', [])  # 从batch中获取回复文本
            ...
            q_lengths = batch['q_length']  # q部分的长度，用于创建labels
            y_ids = batch['response_input_ids'].to(device)
            
            # 提取历史特征
            # history_features = extract_features(qwen_model, history_input_ids, history_attention_mask)
            # # 提取当前问题特征（作为q）
            # q_features = extract_features(qwen_model, current_input_ids, current_attention_mask)
            ...
            outputs = compressor(
                ...
                q_ids=current_input_ids,  # 传递q的token ids
                y_ids=y_ids,  # 传递y的token ids
                qy_embeddings=qy_embeddings,  # 在预测loss部分使用
                qy_attention_mask=qy_attention_mask,
                q_lengths=q_lengths,  # q部分的长度，用于创建labels
            ...
            # 检查是否有NaN
            if torch.isnan(outputs['total_loss']) or torch.isinf(outputs['total_loss']):
                print(f"Warning: NaN or Inf detected in batch {batch_idx}, loss in {outputs['total_loss']} skipping...")
                # 重置优化器状态，避免累积的梯度导致问题
                optimizer.zero_grad()
                continue
            ...
            for name, param in compressor.named_parameters():
                if param.grad is not None and (torch.isnan(param.grad).any() or torch.isinf(param.grad).any()):
                    print(f"Warning: NaN/Inf gradient in {name}, skipping batch {batch_idx}")
                    has_nan_grad = True
                    break
            ...
            # 梯度裁剪，防止梯度爆炸（更严格的裁剪）
            # torch.nn.utils.clip_grad_norm_(compressor.parameters(), max_norm=0.1)
            ...
            # 如果KL loss过大，给出警告
            # if outputs["kl_loss"] > 50:
            #     warning_msg = f"Warning: KL loss too large: {outputs['kl_loss']:.4f} at batch {batch_idx}"
            #     print(warning_msg)
            #     if logger:
...
def main():
    ...
    # 优化器
    lr = 1e-4  # 10.26之前学习率为1e-8
    weight_decay = 0.01
    optimizer = optim.AdamW(compressor.parameters(), lr=lr, weight_decay=weight_decay)
    ...
    # 训练
    train_epoch = 10
    save_interval = 3  # 每3个epoch保存一次
    logger.info("Starting training...")
    print("Starting training...")
    compressor_name = f'compressor_qwen25_15_model_{num_slots}_{lr}_tgt-1_qembed_yid_{summary_max_len}_{seq_max_len}_{max_qy_len}_{recon_weight}_{kl_weight}_{pred_weight}_{train_batch_size}.pth'
    train_compressor(compressor, train_loader, optimizer, device, model, tokenizer, compressor_name, num_epochs=train_epoch, logger=logger, save_dir=save_dir, model_config=model_config, save_interval=save_interval)
