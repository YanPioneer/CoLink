...
class InformationBottleneckLoss(nn.Module):
    ...
    def forward(self, memory_slots, know_slots, z, mu, logvar, summary_ids, summary_attention_mask, q_embeddings, q_mask, q_ids, y_ids, qy_embeddings, qy_attention_mask, q_lengths, frozen_llm, tokenizer, pad_token_id=151643, return_text=False):
        """
        Args:
        ...
        # 1. 重构loss：slots -> 历史摘要
        if return_text:
            recon_loss, reconstructed_texts = self.recon_loss(know_slots, summary_ids, summary_attention_mask, pad_token_id, frozen_llm, tokenizer, return_text=True)
        else:
            recon_loss = self.recon_loss(know_slots, summary_ids, summary_attention_mask, pad_token_id, frozen_llm)
        ...
        pred_loss = self.prediction_loss_with_qyembed_and_yid(memory_slots, know_slots, q_ids, y_ids, frozen_llm, pad_token_id, q_embeddings, q_mask, qy_embeddings, qy_attention_mask, q_lengths)
    ...
    def prediction_loss_with_qyembed_and_yid(self, memory_slots, know_slots, q_ids, y_ids, frozen_llm, pad_token_id, q_embeddings, q_mask, qy_embeddings, qy_attention_mask, q_lengths):
        """slots + q -> 标准回复   qy[pad]
        Args:
        ...
        """
        _, num_slots_memory, _ = memory_slots.shape
        batch_size, num_slots_know, _ = know_slots.shape
        num_slots = num_slots_memory + num_slots_know

        # 首先检查slots是否正常
        if torch.isnan(know_slots).any() or torch.isinf(know_slots).any():
            print("Warning: slots contains NaN or Inf, returning zero loss")
            print(f"Debug - slots stats: min={know_slots.min()}, max={know_slots.max()}, mean={know_slots.mean()}")
            return torch.tensor(0.0, device=know_slots.device, requires_grad=True)
        ...
        full_inputs_embeds = torch.cat([
            memory_slots,
            know_slots,  # slots作为前缀
            qy_embeddings.detach()  # q+y的embeddings
        ], dim=1)

        # 创建attention mask
        # slots部分：全1（总是attend）
        slot_mask = torch.ones(batch_size, num_slots, dtype=torch.long, device=know_slots.device)
        # 拼接qy的mask
        ...
        labels = torch.full((batch_size, total_len), -100, 
                          dtype=torch.long, device=know_slots.device)
        ...
        if torch.isnan(full_inputs_embeds).any():
            print("Warning: NaN detected in full_inputs_embeds!")
            print(f"Debug - slots has NaN: {torch.isnan(know_slots).any()}")
            print(f"Debug - qy_embeddings has NaN: {torch.isnan(qy_embeddings).any()}")
            return torch.tensor(0.0, device=know_slots.device, requires_grad=True)
...
class Know_DialogueCompressionModel(nn.Module):
    """完整的对话压缩模型"""
    ...
    def forward(self, memory_slots, knowledge_features, knowledge_ids, knowledge_attention_mask, summary_ids, summary_attention_mask, q_embeddings, q_mask, q_ids, y_ids, 
        ...
        """
        # 压缩知识
        know_slots, z, mu, logvar = self.variational_slot_attention(knowledge_features, knowledge_attention_mask, training)
